...
def get_protocol_distribution(
    ...
    result = {
        ...
        "protocols": [
            ...
            for protocol, count, bytes_sum in protocols
        ]
    }
    
    return result

@router.get("/{capture_id}/traffic-volume", response_model=TrafficVolume)
def get_traffic_volume(
    capture_id: int,
    interval: str = Query("1m", description="Time interval (1s, 1m, 1h)"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
) -> Any:
    """
    Get traffic volume time series for a capture
    """
    # Check if capture exists
    capture = db.query(Capture).filter(Capture.id == capture_id).first()
    if not capture:
    ...
    # Use SQL time functions to group by interval
    time_series = db.query(
        func.date_trunc('second', Packet.timestamp).label('time_bucket'),
    ...
    points = [
        ...
        for time_bucket, packet_count, bytes_sum in time_series
    ]
...
@router.get("/{capture_id}/top-hosts", response_model=TopHosts)
def get_top_hosts(
    capture_id: int,
    ...
    source_ips = db.query(
        Packet.source_ip.label('ip'),
        func.count(Packet.id).label('packet_count'),
        func.sum(Packet.size).label('bytes_sent')
    ).filter(
        Packet.capture_id == capture_id
    ).group_by(
        Packet.source_ip
    ...
    dest_ips = db.query(
        Packet.destination_ip.label('ip'),
        func.count(Packet.id).label('packet_count'),
        func.sum(Packet.size).label('bytes_received')
    ).filter(
        Packet.capture_id == capture_id
    ).group_by(
    ...
    # Combine results
    source_dict = {ip: {"packets_sent": count, "bytes_sent": bytes_sum, "packets_received": 0, "bytes_received": 0}
                 for ip, count, bytes_sum in source_ips}
    
    for ip, count, bytes_sum in dest_ips:
        if ip in source_dict:
            source_dict[ip]["packets_received"] = count
            source_dict[ip]["bytes_received"] = bytes_sum
        else:
            source_dict[ip] = {"packets_sent": 0, "bytes_sent": 0, "packets_received": count, "bytes_received": bytes_sum}
    ...
    hosts = [
        ...
        for ip, data in source_dict.items()
    ]
...
@router.get("/{capture_id}/top-flows", response_model=TopFlows)
def get_top_flows(
    capture_id: int,
    limit: int = Query(10, description="Number of top flows to return"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
) -> Any:
    """
    Get top flows by traffic volume
    """
    # Check if capture exists
    capture = db.query(Capture).filter(Capture.id == capture_id).first()
    if not capture:
    ...
    # Get flows from Flow model
    flows = db.query(
        Flow
    ).filter(
        Flow.capture_id == capture_id
    ).order_by(
        Flow.bytes_total.desc()
    ).limit(limit).all()
    ...
    for flow in flows:
        result.append({
            ...
            "duration_seconds": (flow.end_time - flow.start_time).total_seconds() if flow.end_time else None
        })
...
@router.get("/{capture_id}/geolocation", response_model=List[GeoLocation])
def get_geolocation(
    capture_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
    ...
    """
    Get geolocation information for IPs in the capture
    """
    # Check if capture exists
    capture = db.query(Capture).filter(Capture.id == capture_id).first()
    if not capture:
    ...
    unique_ips = db.query(
        ...
        Packet.capture_id == capture_id,
        ~Packet.source_ip.like("192.168.%"),  # Filter out private IPs
        ~Packet.source_ip.like("10.%"),
        ~Packet.source_ip.like("172.16.%"),
        ~Packet.source_ip.like("127.%")
    ).group_by(
        Packet.source_ip
    ).union(
        db.query(
            ...
            Packet.capture_id == capture_id,
            ~Packet.destination_ip.like("192.168.%"),  # Filter out private IPs
            ~Packet.destination_ip.like("10.%"),
            ~Packet.destination_ip.like("172.16.%"),
            ~Packet.destination_ip.like("127.%")
        ).group_by(
            Packet.destination_ip
        )
    ).all()
    
    # Lookup geolocations for each IP
    result = []
    for (ip,) in unique_ips:
        ...
        traffic = db.query(
            ...
            (Packet.source_ip == ip) | (Packet.destination_ip == ip),
            Packet.capture_id == capture_id
        ).scalar() or 0
        
        geo_data["traffic_bytes"] = traffic
